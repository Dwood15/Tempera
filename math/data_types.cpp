...
ColorByte::ColorByte(const ColorRGB &other) noexcept :
		blue((unsigned char) (other.blue * 255)),
		green((unsigned char) (other.green * 255)),
		red((unsigned char) (other.red * 255)) {};

ColorByte::ColorByte(const ColorARGB &other) noexcept :
		blue((unsigned char) (other.blue * 255)),
		green((unsigned char) (other.green * 255)),
		red((unsigned char) (other.red * 255)),
		alpha((unsigned char) (other.alpha * 255)) {};
...
Quaternion::Quaternion(const RotationMatrix &matrix) noexcept {
	...
	if (tr > 0) {
		...
		float S = sqrt(1.0f + matrix.v[0].x - matrix.v[1].y - matrix.v[2].z) * 2; // S=4*qx
		...
		float S = sqrt(1.0f + matrix.v[1].y - matrix.v[0].x - matrix.v[2].z) * 2; // S=4*qy
		this->w = (matrix.v[0].z - matrix.v[2].x) / S;
		this->x = (matrix.v[0].y + matrix.v[1].x) / S;
		this->y = 0.25f * S;
		this->z = (matrix.v[1].z + matrix.v[2].y) / S;
	} else {
		float S = sqrt(1.0f + matrix.v[2].z - matrix.v[0].x - matrix.v[1].y) * 2; // S=4*qz
...
float  counter_time_elapsed(const LARGE_INTEGER &before, const LARGE_INTEGER &after) noexcept {
	...
	return static_cast<float>(after.QuadPart - before.QuadPart) / performance_frequency.QuadPart;
}
