...
static unsigned int total_spawns;
...
static void show_spawns() noexcept {
	...
	if (ticks < tickles || tickles == -1) {
		...
		for (auto i = 0u; i < player_spawn_count; i++) {
	...
	if (pdata) {
		...
		float spawn_weight = 1.0f + 3.0f * allies_count;
		...
		for (size_t i = 0; i < total_spawns; i++) {
			...
			if (odata) {
				...
				if (blocked[i]) {
					...
					color.green = (spawn_chance) * 0.5f + 0.5f;
					if (friend_influenced == 1) {
						color.blue = color.green * 0.8f;
					}
					oobject.scale = 0.4f + 0.35f * spawn_chance;
				} else {
					color.red = 1.0f + spawn_chance;
					color.green = 0.0;
					oobject.scale = 0.4f + spawn_chance * 0.4f;
				}

				if (biped) {
					if (blocked[i]) {
						invis_if_biped += 0.35;
						if (invis_if_biped > 1.1) {
							invis_if_biped = 1.1;
						}
					} else {
						invis_if_biped -= 0.075;
						if (invis_if_biped < 0.0) {
							invis_if_biped = 0.0;
						}
					}
				}
