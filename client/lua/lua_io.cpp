...
#define bail_if_not_unlocked if((address < 0x40000000 || address > 0x41B00000) && !script_from_state(state).unlocked) { luaL_error(state,"invalid address specified (script is sandboxed)"); return 0; }
...
void set_io_functions(lua_State *state) noexcept {
    #define lua_register_read_write_int_named(tname,ttype) lua_register(state,"read_" tname,lua_read_int<ttype>); lua_register(state,"write_" tname,lua_write_int<ttype>);
    #define lua_register_read_write_int(ttype) lua_register_read_write_int_named(#ttype, ttype);
    #define lua_register_read_write_float_named(tname,ttype) lua_register(state,"read_" tname,lua_read_float<ttype>); lua_register(state,"write_" tname,lua_write_float<ttype>);
    #define lua_register_read_write_float(ttype) lua_register_read_write_float_named(#ttype, ttype);
