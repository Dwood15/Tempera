...
static void change_colorargb(float val, float max, ColorARGB &color, bool invert) noexcept {
	...
	if (!invert) {
		if (val <= halfway) {
			...
			color.red = 1.0;
			color.green = 1.0f - (val - halfway) / halfway;
		}
	} else {
		if (val <= halfway) {
			color.red = 1.0;
			...
			color.green = 1.0;
			color.red = 1.0f - (val - halfway) / halfway;
		}
	}
...
static void change_colorargb_d(float val, float base, ColorARGB &color, float max_distance) noexcept {
	color.alpha = 1.0;
	color.blue = 0.0;
	float d_from_m = fabs(val - base);
	float halfway = max_distance / 2.0f;
	if (d_from_m < halfway) {
		color.green = 1.0;
		color.red = d_from_m / halfway;
	} else {
		color.red = 1.0;
		color.green = 1.0f - (d_from_m - halfway) / halfway;
	}

	if (color.green > 1.0f) color.green = 1.0;
	if (color.green < 0.0f) color.green = 0.0;
	if (color.red > 1.0f) color.red = 1.0;
	if (color.red < 0.0f) color.red = 0.0;
...
static void show_budget() noexcept {
	...
	if (!console_is_out()) {
		...
		float framerate = static_cast<float>(total_frames) / total_time;
		auto etr = effective_tick_rate();

#define PERCENT_SPACER "   "

		console_out(active == 1 ? "Metric|t     Value|tMax|t%" PERCENT_SPACER : "Metric|t     Value|tMax (Stock Halo)|t%" PERCENT_SPACER);
		...
		float current_tps = buffer_size / total_time;
