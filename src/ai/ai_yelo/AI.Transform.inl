...
#include "../../gamestate/units/units.h"
...
namespace Yelo::AI::Transform {
	...
	void Initialize() {
		Objects::Units::Animations::SetAnimationStateKeyframeHandler(_unit_animation_state_yelo_unit_transforming,
																						 [](const datum_index unit_index, unit_animation_keyframe keyframe) {
																							 g_actor_variant_transform_manager.TriggerUnitTransformKeyframe(
																								 unit_index, keyframe);
																						 });
	}
	...
	void *HS_AITransformActorsByType(void **arguments) {
		...
		result.boolean = g_actor_variant_transform_manager.TransformActorsByType(args->unit_list_index, args->tag_index, args->transform_name,
																										 args->target_name);
