...
namespace Effects {
	...
	NewOnObjectMarker(datum_index
							effect_definition_index,
							datum_index object_index, cstring
							marker_name) {
...
#include "Game/EngineFunctions.Players.inl"
		...
		namespace TagGroups {
			void PredictResources(TagBlock <Yelo::TagGroups::predicted_resource> &resources_block) {
				//TODO: dedi doesn't need to predict resources since it doesn't render bitmaps or play sounds
	...
	namespace blam {
		//////////////////////////////////////////////////////////////////////////
		// actors.c
		__declspec(naked) void actor_delete(datum_index
														actor_index,
														bool is_dead
		) {
		...
		actor_set_active(const datum_index actor_index, const bool active) {
		...
		actor_set_dormant(const datum_index actor_index, const bool dormant) {
		...
		actor_delete_props(const datum_index actor_index) {
		...
		actor_freeze(const datum_index actor_index) {
		...
		actor_braindead(const datum_index actor_index, const bool braindead) {
		...
		actor_create_for_unit(const bool is_swarm, const datum_index unit_index, const datum_index actor_variant, const datum_index encounter_index, const int32 squad_index,
									 const int32 arg7,
		...
		actor_action_change(const datum_index actor_index, const Enums::actor_action new_action_type, const byte *new_action_data) {
		...
		// ai_script.c
		__declspec(naked) void
		ai_scripting_attach_free(datum_index
										 unit_index,
			...
			mov
			eax, actor_variant_definition_index
			push
			unit_index
			call
			FUNCTION
	...
	namespace blam {
		...
		bool
		cache_file_read_request(/*datum_index tag_index,*/
			uint32
			offset,
			uint32 size,
			void *buffer,
			...
			source) {
			return
				Engine::Cache::FileReadRequest(offset, size, buffer, params, block, source
				);
		}
		...
		// pc_sound_cache.c
		bool
		sound_cache_sound_request(TagGroups::s_sound_permutation
										  *sound_perm,
										  bool block_thread,
										  bool load,
										  bool reference
			...
			return
				Engine::SoundCacheRequestSound(sound_perm, block_thread, load, reference
				);
		}
		...
		texture_cache_bitmap_get_hardware_format(TagGroups::s_bitmap_data
															  *bitmap,
			// block
															  bool block_thread,
															  bool load
			...
			return
				Engine::TextureCacheBitmapGetHardwareFormat(bitmap, block_thread, load
				);
		}
	...
	namespace blam {
		...
		// game_engine.c
		datum_index
		find_closest_player_index(datum_index
										  player_index) {
			return
				Engine::Players::FindClosestPlayerIndex(player_index);
		}

		__declspec(naked) void
		game_engine_rasterize_message(wcstring
												message,
												real alpha
		) {
#if !PLATFORM_IS_DEDI
		...
		__declspec(naked) void
		game_engine_play_multiplayer_sound(datum_index
													  player_index,
		...
		// players.c
		__declspec(naked) void
		player_examine_nearby_vehicle(datum_index
												player_index,
		...
		__declspec(naked) void
		player_set_action_result(datum_index
										 player_index,
										 datum_index action_object_index, int32
										 action_result,
		...
		__declspec(naked) void
		player_set_action_result_to_network(datum_index
														player_index,
														datum_index action_object_index, int32
														action_result_type,
														int32 action_result, int32
														action_seat_index,
		...
		datum_index
		player_index_from_unit_index(datum_index
											  unit_index) {
			return
				Engine::Players::IndexFromUnitIndex(unit_index);
		}

		bool
		player_teleport(datum_index
							 player_index,
							 datum_index source_unit_index,
							 const real_point3d &position
		) {
			return
				Engine::Players::Teleport(player_index, position, source_unit_index
				);
		}

		void
		player_over_shield_screen_effect(datum_index
													player_index) {
			Engine::Players::ScreenEffectOvershield(player_index);
		}

		void
		player_active_camo_screen_effect(datum_index
													player_index) {
			Engine::Players::ScreenEffectCamo(player_index);
		}

		void
		player_health_pack_screen_effect(datum_index
													player_index) {
		...
		player_control_get_unit_camera_info(const int16 player_index, Players::s_unit_camera_info &camera_info) {
#if !PLATFORM_IS_DEDI
	...
	namespace blam {
		...
		void
		hs_effect_new_from_object_marker(datum_index
													effect_definition_index,
													datum_index object_index, cstring
													marker_name) {
	...
	namespace blam {
		//////////////////////////////////////////////////////////////////////////
		// hud.c
		__declspec(naked) wcstring
		hud_get_item_string(int16
								  reference_index) {
		...
		// hud_chat.c
		void
		hud_chat_to_network(int32
								  player_number,
								  long_enum chat_type, wcstring
								  text) {
		...
		__declspec(naked) void
		hud_chat_display_message(wcstring
										 message) {
		...
		// hud_draw.c
		__declspec(naked) void
		hud_draw_meter(int16 /*local_player_index*/, TagGroups::s_hud_absolute_placement
		*placement,
							TagGroups::s_hud_element_meter *element,
							byte
							arg_C,
							byte arg_10, long_flags
							flags,
							real arg_18, real
							arg_1C) {
		...
		__declspec(naked) void
		hud_draw_numbers(int16
							  local_player_index,
							  TagGroups::s_hud_absolute_placement *placement, TagGroups::s_hud_element_number
							  *element,
							  int32 number, int16
							  arg_10,
							  long_flags flags, game_ticks_t
							  first_render_time,
							  real arg_1C
		) {
		...
		__declspec(naked) void
		hud_draw_static_element(int16
										local_player_index,
										TagGroups::s_hud_absolute_placement *placement, TagGroups::s_hud_element_static
										*element,
										long_flags flags, game_ticks_t
										first_render_time) {
		...
		// ui_video_screen.c
		void
		ui_video_screen_add_resolution(uint32
												 width,
	...
	namespace blam {
		...
		weapon_stop_reload(const datum_index weapon_index) {
		...
		first_person_weapon_message_from_unit(const datum_index unit_index, const int32 weapon_message_type) {
		...
		weapon_get_first_person_animation_time(const datum_index weapon_index, const int16 frame_type, Enums::first_person_weapon_animation animation,
															const int32 arg3) {
	...
	namespace blam {
		...
		// console.c
		__declspec(naked) bool
		console_process_command(long_flags
										access_flags,
										cstring command
		) {
		...
		console_printf_impl(bool clear_screen, cstring format) {
		...
		console_printf(bool clear_screen, cstring format, ...) {
		...
		static __declspec(naked) void
		console_response_printf_impl(BOOL
											  clear_screen,
											  cstring format
		) {
		...
		console_response_printf(bool clear_screen, cstring format, ...) {
			char local[k_engine_function_string_buffer_size];
			memset(local, 0, k_engine_function_string_buffer_size);
		...
		void
		console_warning(cstring
							 format, ...) {
			char local[k_engine_function_string_buffer_size];
			memset(local,
					 0, k_engine_function_string_buffer_size);
		...
		bool console_process_remote_command(cstring command, int32 machine_index) {
			...
			Console::TerminalGlobals()->rcon_machine_index =
				NONE;
	...
	namespace blam {
		...
		__declspec(naked) real

		periodic_function_evaluate(Enums::periodic_function function_type, real input) {
			...
			input
			sub
			esp, 4 * 2         // allocate space for the 'input' parameter
			fstp
			qword
			...
			FUNCTION
			add
			esp, 4 * 2         // deallocate. double type consumes two DWORDs of stack
			API_FUNC_NAKED_END_NO_STACK_POP()
		}

		__declspec(naked) real

		transition_function_evaluate(Enums::transition_function function_type, real input) {
	...
	namespace blam {
		...
		//////////////////////////////////////////////////////////////////////////
		// data.c
		__declspec(naked) s_data_array
		*
		data_new(cstring
					name,
					int32 maximum_count, size_t
					datum_size) {
		...
		void
		data_dispose(s_data_array
						 *data) {
			if (data != nullptr) {
				CAST_PTR(Yelo::Memory::s_data_array
								*,
							GlobalFree(data)
				);
			}
		...
		void *
		datum_try_and_get(s_data_array
								*data,
								datum_index index
		) {
	...
	namespace blam {
		...
		model_find_marker(const datum_index render_model_definition_index, cstring marker_name) {
			static const uintptr_t CALL_ADDRESS = Engine::GET_FUNC_PTR(MODEL_FIND_MARKER);

			API_FUNC_NAKED_START()
			mov
			eax, render_model_definition_index
			push
			render_model_definition_index
			push
			marker_name
			call
			CALL_ADDRESS
		...
		// model_animations.c
		int16
		animation_choose_random_permutation_internal(long_enum
																	render_or_affects_game_state,
																	datum_index animation_graph_index, int32
																	animation_index) {
			return
				Engine::AnimationPickRandomPermutation(render_or_affects_game_state, animation_graph_index, animation_index
				);
		}
	};
	//////////////////////////////////////////////////////////////////////////
	...
	namespace blam {
		...
		// damage.c
		bool
		object_restore_body(datum_index
								  object_index) {
			return
				Engine::Objects::RestoreBody(object_index);
		}

		void
		object_deplete_body(datum_index
								  object_index) {
			Engine::Objects::DepleteBody(object_index);
		}

		void
		object_deplete_shield(datum_index
									 object_index) {
			Engine::Objects::DepleteShield(object_index);
		}

		void
		object_double_charge_shield(datum_index
											 object_index) {
			Engine::Objects::DoubleChargeShield(object_index);
		}

		void
		area_of_effect_cause_damage(s_damage_data
											 &data, datum_index) {
			// TODO
		}

		void
		object_cause_damage(s_damage_data
								  &data,
								  datum_index damaged_object_index,
								  int16
								  node_index,
								  int16 region_index, int16
		...
		// index_resolution.c
		int32
		index_resolution_table_translate(MessageDeltas::s_index_resolution_table
													&table,
			...
			return
				Engine::Networking::TranslateIndex(table, local_index
				);
#else
			return 0;
#endif
		...
		// objects.c
		void
		object_reset(datum_index
						 object_index) {
			Engine::Objects::Reset(object_index);
		}

		void
		object_set_position(datum_index
								  object_index,
								  __in_opt
								  real_point3d
		...
		void
		object_set_position_network(datum_index
											 object_index,
		...
		void
		object_translate(datum_index
							  object_index,
							  const real_point3d &new_position, __in_opt
							  const s_scenario_location
		...
		void
		object_placement_data_new(s_object_placement_data
										  &data,
										  datum_index object_definition_index, datum_index
										  owner_object_index) {
		...
		datum_index
		object_new(s_object_placement_data
					  &data) {
			return
				Engine::Objects::New(data);
		}

		datum_index
		object_new_with_role(s_object_placement_data
									&data,
									Enums::networked_datum role
		) {
			return
				Engine::Objects::NewWithRole(data, role
				);
		}

		void
		object_delete_to_network(datum_index
										 object_index) {
			Engine::Networking::EncodeObjectDeletionMessage(object_index);
		}

		void
		object_delete(datum_index
						  object_index) {
			Engine::Objects::Delete(object_index);
		}

		void
		object_reconnect_to_map(datum_index
										object_index,
										__in_opt
										s_scenario_location
		...
		void
		object_disconnect_from_map(datum_index
											object_index) {
		...
		object_get_marker_by_name(const datum_index object_index, cstring marker_name, s_object_marker *markers, const int16 maximum_marker_count) {
		...
		void
		object_get_orientation(datum_index
									  object_index,
									  __out_opt
									  real_vector3d
		...
		s_scenario_location &
		object_get_location(datum_index
								  object_index,
								  __out
								  s_scenario_location
								  &
								  ...
								  return_location) {
			return
				Engine::Objects::GetLocation(object_index, return_location
				);
		}

		void
		object_start_interpolation(datum_index
											object_index,
		...
		s_object_data *
		object_iterator_next(s_object_iterator
									&iter) {
			return
				Engine::Objects::IteratorNext(iter);
		}

		int16
		objects_in_sphere(Flags::objects_find_flags
								find_flags,
								long_flags object_type_flags,
								const s_scenario_location &location,
								const real_point3d &center, real
								radius,
								datum_index object_indices[], int16
								maximum_object_indices) {
			return
				Engine::Objects::FindInSphere(find_flags, object_type_flags,
														location, center, radius, object_indices, maximum_object_indices
				);
		}

		void
		object_definition_predict(datum_index
										  object_index) {
			Engine::Objects::DefinitionPredict(object_index);
		}

		bool
		object_header_block_allocate(datum_index
											  object_index,
											  size_t block_reference_offset, size_t
											  size) {
			return
				Engine::Objects::HeaderBlockAllocate(object_index, block_reference_offset, size
				);
		}

		void
		objects_scripting_set_scale(datum_index
											 object_index,
											 real scale, int32
											 ticks) {
		...
		real
		object_get_level_of_detail_pixels(datum_index
													 object_index) {
			return
				Engine::Objects::GetLevelOfDetailPixels(object_index);
		}

		void
		object_render_state_refresh(datum_index
											 object_render_state_index,
											 datum_index object_index, real
		...
		object_destroy(const datum_index object_index) {
	...
	namespace blam {
		...
		// collisions.c
		__declspec(naked) bool
		collision_test_vector(long_flags
									 flags,
									 real_point3d &location, real_vector3d
									 &vector,
									 datum_index object_index, Physics::s_collision_result
									 &collision) {
	...
	namespace blam {
		...
		bool
		rasterizer_set_texture_bitmap_data(_enum
													  stage,
													  TagGroups::s_bitmap_data *bitmap
		) {
			return
				Engine::SetTextureSamplerStage(bitmap, stage
				);
		}
	};
	//////////////////////////////////////////////////////////////////////////
	...
	namespace blam {
		...
		render_window(const uint16 local_player_index, void *render_camera, void *render_frustum, void *rasterizer_camera, void *rasterizer_frustum,
						  void *rasterizer_target, const bool is_mirror) {
			CAST_PTR(void(*)(
							const uint16, void*, void*, void*, void*, void*, const bool), Engine::GET_FUNC_PTR(RENDER_WINDOW))
	...
	namespace blam {
		...
		__declspec(naked) bool
		scenario_switch_structure_bsp(int16
												bsp_index) {
		...
		bool
		scenario_trigger_volume_test_point(int32
													  trigger_volume_index,
													  const real_point3d &point
		) {
			return
				Engine::Scenario::TriggerVolumeTestPoint(trigger_volume_index, point
				);
		}

		bool
		scenario_trigger_volume_test_object(int32
														trigger_volume_index,
														datum_index object_index
		) {
			return
				Engine::Scenario::TriggerVolumeTestObject(trigger_volume_index, object_index
				);
		}
	};
	//////////////////////////////////////////////////////////////////////////
	...
	namespace blam {
		...
		// shell_windows.c
		__declspec(naked) bool
		shell_get_command_line_argument(cstring
												  param,
												  _Out_opt_ cstring
	...
	namespace blam {
		...
		void
		tag_iterator_new(TagGroups::s_tag_iterator
							  &iter,
		...
		datum_index
		tag_iterator_next(TagGroups::s_tag_iterator
								&iter) {
	...
	namespace blam {
		//////////////////////////////////////////////////////////////////////////
		// text_group.c
		wcstring unicode_string_list_get_string(datum_index
															 unicode_string_list_definition_index,
															 int32 reference_index
		) {
			return
				Engine::TagGroups::UnicodeStringListGetString(unicode_string_list_definition_index, reference_index
				);
		}
	};
	//////////////////////////////////////////////////////////////////////////
	...
	namespace blam {
		...
		unit_update(const datum_index unit_index) {
		...
		void
		unit_set_animation(datum_index
								 unit_index,
								 datum_index animation_graph_index, int32
								 animation_index) {
		...
		unit_animation_start_action(const datum_index unit_index, const Enums::unit_replacement_animation_state action_type) {
		...
		int16
		unit_find_nearby_seat(datum_index
									 unit_index,
									 datum_index
									 target_unit_index, __out
									 int16 &

									 parent_seat_index
		) {
			return
				Engine::Objects::UnitFindNearbySeat(unit_index, target_unit_index, parent_seat_index
				);
		}

		bool
		unit_can_enter_seat(datum_index
								  unit_index,
								  datum_index
								  target_unit_index, int16
								  seat_index,
								  __out
								  datum_index
								  ...
								  *unit_in_seat_index) {
			return
				Engine::Objects::UnitCanEnterSeat(unit_index, target_unit_index, seat_index, unit_in_seat_index
				);
		}

		bool
		unit_enter_seat(datum_index
							 unit_index,
							 datum_index target_unit_index, int32
							 seat_index) {
			return
				Engine::Objects::UnitEnterSeat(unit_index, target_unit_index, seat_index
				);
		}

		void
		unit_get_camera_position(datum_index
										 unit_index,
										 __out
										 real_point3d
		...
		unit_kill(const datum_index unit_index) {
		...
		void
		unit_open(datum_index
					 unit_index) {
			Engine::Objects::UnitOpen(unit_index);
		}

		void
		unit_close(datum_index
					  unit_index) {
			Engine::Objects::UnitClose(unit_index);
		}

		void
		unit_scripting_exit_vehicle(datum_index
											 unit_index) {
		...
		unit_try_and_exit_seat(const datum_index unit_index) {
		...
		unit_detach_from_parent(const datum_index unit_index) {
		...
		void
		unit_exit_seat_end(datum_index
								 unit_index,
		...
		unit_can_see_point(const datum_index unit_index, const real_point3d *point, const real view_radians) {
		...
		int16 unit_get_custom_animation_time(datum_index
														 unit_index) {
			return
				Engine::Objects::UnitGetCustomAnimationTime(unit_index);
		}

		__declspec(naked) bool

		unit_start_user_animation(const datum_index unit_index, const datum_index animation_definition_index, cstring animation_name,
			...
			eax, animation_name
			mov
			edi, animation_definition_index
			xor ebx, ebx
			mov
			bl, interpolate
			push
		...
		unit_animation_set_state(const datum_index unit_index, const Enums::unit_animation_state state) {
		...
		unit_drop_current_weapon(const datum_index unit_index, const bool force) {
		...
		unit_inventory_get_weapon(const datum_index unit_index, const int16 index) {
		...
		unit_ready_desired_weapon(const datum_index unit_index, const bool force) {
		...
		unit_throw_grenade_release(const datum_index unit_index, const sbyte keyframe) {
		...
		unit_cause_player_melee_damage(const datum_index unit_index) {
		...
		unit_damage_aftermath(const datum_index unit_index, const Objects::s_damage_data *damage_data, const _enum damage_flags,
									 const real shield_amount, const real body_amount, void *arg6,
		...
		unit_scripting_set_current_vitality(const datum_index unit_index, const real health, const real shield) {
		...
		unit_set_actively_controlled(const datum_index unit_index, const bool controlled) {
