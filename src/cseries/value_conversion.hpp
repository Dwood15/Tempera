...
namespace Yelo {
	namespace ValueConversion {
		...
		bool FromString(const char *string, argb_color &output) {
			...
			output.rgb.red   = (byte) red;
			output.rgb.green = (byte) green;
			output.rgb.blue  = (byte) blue;
		...
		bool FromString(const char *string, real_argb_color &output) {
			return FromStringImpl("%f %f %f %f", 4, string, &output.rgb.red, &output.rgb.green, &output.rgb.blue, &output.alpha);
		}
	};
};
