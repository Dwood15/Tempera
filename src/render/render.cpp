...
namespace render {
	int main_get_window_count_override() {
		...
		auto player_count = CurrentEngine.GetCore()->players_globals->local_player_count;
	...
	namespace overrides {
		...
		void override_all() {
			for (unsigned int i = 0; i < std::size(override_function_call_list); i++) {
				...
				auto addr    = ::calls::calc_addr_offset(current, (int) &render::main_get_window_count_override);
				calls::patchValue<uintptr_t>(current, addr);
			}
		}
	};
